 ...
 */
async function fileExists(filename, projectRoot = process.cwd()) {
    try {
 ...
 */
async function readPackageJson(projectRoot = process.cwd()) {
    try {
        const content = await fs.readFile(path.join(projectRoot, 'package.json'), 'utf8');
        return JSON.parse(content);
    } catch {
        return null;
    }
...
/**
 * Detect E2E test strategy based on project type.
 *
 * Detection order:
 * 1. Playwright config exists → playwright
 * 2. Cypress config exists → cypress
 * 3. package.json has web framework (react/vue/angular) → playwright
 * 4. OpenAPI/Swagger spec exists → api
 * 5. package.json has bin field → cli
 * 6. Default → bash
 *
 * @param {string} [projectRoot] - Project root directory
 * @returns {Promise<{type: string, command: string, description: string}>}
 */
async function detectE2EStrategy(projectRoot = process.cwd()) {
    // Check for Playwright
    if (
        (await fileExists('playwright.config.ts', projectRoot)) ||
        (await fileExists('playwright.config.js', projectRoot)) ||
        (await fileExists('playwright.config.mjs', projectRoot))
    ) {
        return {
            type: 'playwright',
            command: 'npx playwright test',
            description: 'Browser E2E tests via Playwright'
        };
    }

    // Check for Cypress
    if (
        (await fileExists('cypress.config.ts', projectRoot)) ||
        (await fileExists('cypress.config.js', projectRoot)) ||
        (await fileExists('cypress.config.mjs', projectRoot))
    ) {
        return {
            type: 'cypress',
            command: 'npx cypress run',
            description: 'Browser E2E tests via Cypress'
        };
    }
    ...
    if (pkg) {
        ...
        // Web frameworks → suggest Playwright
        if (deps.react || deps.vue || deps.angular || deps['@angular/core'] || deps.svelte || deps.next || deps.nuxt) {
            return {
                type: 'playwright',
                command: 'npx playwright test',
                description: 'Browser E2E tests via Playwright (web framework detected)'
            };
        }
        ...
        if (pkg.bin) {
            return {
                type: 'cli',
                command: 'npm run test:e2e || ./test/e2e.sh',
                description: 'CLI E2E tests via Bash'
            };
        }
    }
    ...
    if (
        (await fileExists('openapi.yaml', projectRoot)) ||
        (await fileExists('openapi.yml', projectRoot)) ||
        (await fileExists('openapi.json', projectRoot)) ||
        (await fileExists('swagger.yaml', projectRoot)) ||
        (await fileExists('swagger.yml', projectRoot)) ||
        (await fileExists('swagger.json', projectRoot))
    ) {
        return {
            type: 'api',
            command: 'npm run test:e2e || ./test/e2e-api.sh',
            description: 'API E2E tests via curl/httpie'
        };
    }
    ...
    return {
        type: 'bash',
        command: 'npm run test:e2e || ./test/e2e.sh',
        description: 'System E2E tests via Bash'
    };
}

/**
 * Run E2E tests based on detected strategy.
 *
 * @param {Object} [options]
 * @param {string} [options.projectRoot] - Project root directory
 * @param {string} [options.customCommand] - Override detected command
 * @param {(message: string) => void} [options.onProgress] - Progress callback
 * @returns {Promise<{success: boolean, output: string, strategy: Object}>}
 */
async function runE2ETests(options = {}) {
    const { projectRoot = process.cwd(), customCommand, onProgress } = options;

    const strategy = await detectE2EStrategy(projectRoot);
    const command = customCommand || strategy.command;

    if (onProgress) {
        onProgress(`Running E2E tests: ${strategy.description}`);
    ...
    try {
        ...
        if (onProgress) onProgress('E2E tests passed');
        return { success: true, output, strategy };
    } catch (error) {
        const output = error.stdout || error.stderr || error.message || 'E2E tests failed';
        if (onProgress) onProgress(`E2E tests failed: ${output.slice(0, 200)}`);
        return { success: false, output, strategy };
    }
}
